...
public class GunInventory : MonoBehaviour {
	...
	[HideInInspector]
	public float switchWeaponCooldown; // time to prevent changing weapons too fast
	 ...
	 */
	void Awake(){
		StartCoroutine("UpdateIconsFromResources"); // update icons from resources

		StartCoroutine ("SpawnWeaponUponStart");// After 0.5 seconds equip a weapon
	...
	*/
    ...
    public void TakeAmmo()
    {
		...
		GunScript gunScript =  gameObject.GetComponentInChildren<GunScript>();
		if(gunScript!= null)
		{
			gunScript.TakeAmmo(ammo);
		}
    }
    ...
    IEnumerator Spawn(int _redniBroj)
    {
        ...
        if (currentGun)
        {
            if (currentGun.name.Contains("Gun"))
            {
                ...
                AssignHandsAnimator(currentGun); // Gọi lại hàm gán animator và vị trí
				Debug.Log("CurrentGun:   " + currentGun.name);
            }
    //        else if (currentGun.name.Contains("Sword"))
    //        {
				//Destroy(currentGun);
	...
	*/
    void AssignHandsAnimator(GameObject _currentGun)
    {
        ...
        {
            // Đối với súng, tiếp tục giữ như cũ
            currentHAndsAnimator = currentGun.GetComponent<GunScript>().handsAnimator;
            currentHAndsAnimator.SetLayerWeight(currentHAndsAnimator.GetLayerIndex("Sword"), 0);
            currentHAndsAnimator.SetLayerWeight(currentHAndsAnimator.GetLayerIndex("Default"), 1);

            // Gắn súng vào tay người chơi
            _currentGun.transform.SetParent(transform);
            _currentGun.transform.localPosition = Vector3.zero;
        }
   //     else if (_currentGun.name.Contains("Sword"))
   //     {
   //         // Đối với kiếm, tương tự như súng
   //         currentHAndsAnimator = currentGun.GetComponent<SwordScript>().handsAnimator;
			//currentHAndsAnimator.SetLayerWeight(currentHAndsAnimator.GetLayerIndex("Default"), 0);
			//currentHAndsAnimator.SetLayerWeight(currentHAndsAnimator.GetLayerIndex("Sword"), 1);

			//// Gắn kiếm vào tay người chơi và cập nhật vị trí sao cho kiếm di chuyển linh hoạt theo người chơi
			//_currentGun.transform.SetParent(transform);
   //         _currentGun.transform.localPosition = Vector3.zero; // Đặt kiếm vào vị trí trung tâm tay người chơi

			//// Cập nhật góc quay kiếm theo góc quay của camera và người chơi
			//_currentGun.transform.localRotation = Quaternion.identity; // Đảm bảo rằng kiếm không bị xoay lệch
			//Debug.Log("Fix loiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii");
