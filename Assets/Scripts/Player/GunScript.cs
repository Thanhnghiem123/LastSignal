...
public class GunScript : MonoBehaviour {
    ...
    {
        public static Vector3 lastSoundPosition; // Lưu vị trí âm thanh cuối cùng
        public static bool soundMade = false; // Cờ để báo âm thanh vừa phát
    ...
    public class WeaponAmmo
    {
        ...
        [Tooltip("Type of weapon that we are using.")]
    ...
    public WeaponAmmo[] weapons = new WeaponAmmo[2];
    [HideInInspector]
	...
	void Start()
	{
		InitialAmmo(currentStyle == GunStyles.automatic ? 0 : 1);
	}

    public void TakeAmmo(float amount)
    {
        // Tăng đạn cho cả hai loại súng
        weapons[0].bulletsIHave += amount;       // automatic
        weapons[1].bulletsIHave += amount / 5;   // nonautomatic
                                                 // Lưu cả hai loại đạn vào PlayerPrefs
        SaveAllAmmo();
        // Cập nhật súng hiện tại
        InitialAmmo(currentStyle == GunStyles.automatic ? 0 : 1);
    }

	public void InitialAmmo(int GunIndex)
	{
		weapons[GunIndex].weaponType = GunIndex == 0 ? "Automatic" : "Nonautomatic";
		// Lấy từ PlayerPrefs nếu có, nếu không thì dùng giá trị từ weapons
		bulletsIHave = PlayerPrefs.GetFloat("BulletsIHave_" + GunIndex, weapons[GunIndex].bulletsIHave);
		bulletsInTheGun = PlayerPrefs.GetFloat("BulletsInTheGun_" + GunIndex, weapons[GunIndex].bulletsInTheGun);
		amountOfBulletsPerLoad = PlayerPrefs.GetFloat("AmountOfBulletsPerLoad_" + GunIndex, weapons[GunIndex].amountOfBulletsPerLoad);
		// Đồng bộ lại weapons
		weapons[GunIndex].bulletsIHave = bulletsIHave;
		weapons[GunIndex].bulletsInTheGun = bulletsInTheGun;
		weapons[GunIndex].amountOfBulletsPerLoad = amountOfBulletsPerLoad;
	}
	public void SetNewAmmo(int GunIndex)
	{
		// Xác định loại súng
		weapons[GunIndex].weaponType = GunIndex == 0 ? "Automatic" : "Nonautomatic";

		// Sử dụng giá trị mặc định từ weapons để khởi tạo
		bulletsIHave = weapons[GunIndex].bulletsIHave;
		bulletsInTheGun = weapons[GunIndex].bulletsInTheGun;
		amountOfBulletsPerLoad = weapons[GunIndex].amountOfBulletsPerLoad;
		...
		Debug.Log($"Initialized ammo for {weapons[GunIndex].weaponType}: " +
				  $"BulletsIHave = {bulletsIHave}, " +
				  $"BulletsInTheGun = {bulletsInTheGun}, " +
				  $"AmountOfBulletsPerLoad = {amountOfBulletsPerLoad}");
	}

	private void SaveCurrentAmmo()
    {
        // Lưu trạng thái súng hiện tại
        int currentIndex = (currentStyle == GunStyles.automatic ? 0 : 1);
        PlayerPrefs.SetFloat("BulletsIHave_" + currentIndex, bulletsIHave);
        PlayerPrefs.SetFloat("BulletsInTheGun_" + currentIndex, bulletsInTheGun);
        PlayerPrefs.SetFloat("AmountOfBulletsPerLoad_" + currentIndex, amountOfBulletsPerLoad);
        weapons[currentIndex].bulletsIHave = bulletsIHave;
        weapons[currentIndex].bulletsInTheGun = bulletsInTheGun;
        weapons[currentIndex].amountOfBulletsPerLoad = amountOfBulletsPerLoad;
        PlayerPrefs.Save();
    }

    // Phương thức mới để lưu cả hai loại đạn
    private void SaveAllAmmo()
    {
        // Lưu cho automatic (index 0)
        PlayerPrefs.SetFloat("BulletsIHave_0", weapons[0].bulletsIHave);
        PlayerPrefs.SetFloat("BulletsInTheGun_0", weapons[0].bulletsInTheGun);
        PlayerPrefs.SetFloat("AmountOfBulletsPerLoad_0", weapons[0].amountOfBulletsPerLoad);

        // Lưu cho nonautomatic (index 1)
        PlayerPrefs.SetFloat("BulletsIHave_1", weapons[1].bulletsIHave);
        PlayerPrefs.SetFloat("BulletsInTheGun_1", weapons[1].bulletsInTheGun);
        PlayerPrefs.SetFloat("AmountOfBulletsPerLoad_1", weapons[1].amountOfBulletsPerLoad);
	...
	*/
    void FixedUpdate(){
		...
		if(Input.GetAxis("Fire2") != 0 && !reloading && !meeleAttack){
			...
			{
                secondCamera.nearClipPlane = nearClipPlane;
            }
		...
		//if not aiming
            ...
            secondCamera.nearClipPlane = 0.01f;
        }
	...
	*/
    ...
    void PositionGun()
    {
        // Cập nhật vị trí cho súng (vị trí của súng sẽ theo camera)
        transform.position = Vector3.SmoothDamp(transform.position,
            mainCamera.transform.position - (mainCamera.transform.right * (currentGunPosition.x + currentRecoilXPos)) +
            (mainCamera.transform.up * (currentGunPosition.y + currentRecoilYPos)) +
            (mainCamera.transform.forward * (currentGunPosition.z + currentRecoilZPos)), ref velV, 0);
	...
	*/
    ...
    [Tooltip("Thời gian chờ giữa các phát bắn ở chế độ không tự động (giây).")]
    public float nonAutoDelayTime = 1.0f; // Mặc định là 1 giây, có thể chỉnh trong Inspector
    ...
    void Shooting()
    {
        ...
        // Giảm thời gian chờ theo thời gian thực (giây)
	...
	public float secondCameraZoomRatio_aiming;
    [Tooltip("nearClipping of Camera")]
	...
	*/
	...
	[Tooltip("Smoke appear after shoot")]
        ...
        if (waitTillNextFire <= 0 && !reloading && pmS.maxSpeed < 5)
        {
            if (bulletsInTheGun > 0)
            {
                ...
                if (player != null)
                {
                    if (holdSmoke != null)
                    {
                        holdSmoke = Instantiate(smoke, muzzelSpawn.transform.position, muzzelSpawn.transform.rotation) as GameObject;
                        holdSmoke.transform.parent = muzzelSpawn.transform; // Align smoke with the gun barrel
                        holdSmoke.transform.rotation = Quaternion.LookRotation(player.transform.forward); // Set the smoke's rotation to match the player's forward direction
                    }
                }
                else
                {
                ...
                // Đặt thời gian chờ dựa trên chế độ bắn
                if (currentStyle == GunStyles.nonautomatic)
                {
                    waitTillNextFire = nonAutoDelayTime; // Thời gian chờ cố định cho súng bắn tỉa (ví dụ: 1 giây)
                }
                else if (currentStyle == GunStyles.automatic)
                {
                    waitTillNextFire = 1 / roundsPerSecond; // Thời gian chờ dựa trên tốc độ bắn tự động
                }
